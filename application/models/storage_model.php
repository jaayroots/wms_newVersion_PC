...
//        Select for Table STK_M_Storage
        ...
        $this->db->from("STK_M_Storage");
        $this->db->where("STK_M_Storage.Active", 'Y');
...
//        Select for Table STK_M_Storage, STK_M_Storage_Type, STK_M_Warehouse
        $this->db->select("STK_M_Storage.Storage_Id AS Id
            , STK_M_Warehouse.Warehouse_Code
            , STK_M_Storage_Type.StorageType_NameEn
            , STK_M_Storage.Storage_NameEn
            , STK_M_Storage.Storage_Height
            , STK_M_Storage.Storage_Width
            , STK_M_Storage.Storage_Lenght
            , STK_M_Storage.Storage_Row
            , STK_M_Storage.Storage_Column
            , STK_M_Storage.Storage_Level
            , STK_M_Storage.Location_Height            
            , STK_M_Storage.Location_Width
            , STK_M_Storage.Location_Lenght
            , STK_M_Storage.Max_Capacity
            , CASE WHEN STK_M_Storage.Active IN ('Y') THEN 'YES' ELSE 'NO' END AS Active");
//        $this->db->from("STK_M_Storage INNER JOIN STK_M_Storage_Type 
//            ON STK_M_Storage.StorageType_Id = STK_M_Storage_Type.StorageType_Id 
//            INNER JOIN STK_M_Warehouse ON STK_M_Storage.Warehouse_Id = STK_M_Warehouse.Warehouse_Id");
        $this->db->from("STK_M_Storage");
        $this->db->join("STK_M_Storage_Type", "STK_M_Storage.StorageType_Id = STK_M_Storage_Type.StorageType_Id", "INNER");
        $this->db->join("STK_M_Warehouse", "STK_M_Storage.Warehouse_Id = STK_M_Warehouse.Warehouse_Id AND STK_M_Warehouse.Active ='Y'", "INNER");
        if ($All == FALSE):
            $this->db->where("STK_M_Storage.Active", 'Y');
        endif;
        $this->db->order_by("STK_M_Storage.Storage_Id");
